  ...
  return (
    <Sheet isOpen={isOpen} onClose={onClose} variant="center" className="w-full max-w-sm bg-surface-container-low border border-outline-variant/20 rounded-2xl shadow-2xl p-5">
      ...
      <div className="flex flex-col gap-1 max-h-64 overflow-y-auto scrollbar-hide mb-4">
        {isLoading && (
          <p className="text-sm text-on-surface-variant py-4 text-center">Loading collections…</p>
        )}
          ...
          return (
            ...
            >
              <div className="flex flex-col min-w-0">
                ...
                <span className="text-[11px] font-jetbrains text-on-surface-variant">{c._count?.items ?? 0} items</span>
              </div>
      ...
      <div className="flex items-center gap-2 border-t border-outline-variant/10 pt-4">
        ...
        />
        <Button onClick={handleCreate} disabled={!newTitle.trim() || isCreating} className="flex items-center gap-1.5 shrink-0">
          <FiPlus /> Create
        </Button>
      </div>
    </Sheet>
