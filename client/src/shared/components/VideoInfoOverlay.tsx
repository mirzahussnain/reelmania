...
export const VideoInfoOverlay: React.FC<VideoInfoOverlayProps> = ({ video }) => {
  ...
  return (
    <section className="absolute left-4 bottom-24 lg:bottom-12 w-[calc(100%-5rem)] lg:w-[calc(100%-6rem)] flex flex-col justify-end items-start z-20 pointer-events-none">
      
      <div className="w-full relative">
        <h2 className="flex flex-wrap items-center gap-2 font-semibold text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] pointer-events-auto">
        ...
        <div className="w-full relative pointer-events-auto mt-2 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
          <div className={cn(
            "w-full text-sm text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] transition-all duration-300 ease-in-out",
            ...
            <div className={cn(
              ...
              {video.hashtags.map((hashtag, index) => (
                <span className="font-semibold text-xs drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]" key={index}>
                  #{hashtag}
                </span>
              ))}
            </div>
          ...
          <button 
            className="absolute bottom-0 right-0 font-bold text-sm text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)] bg-transparent px-1 hover:underline"
            onClick={(e) => {
              e.stopPropagation();
