...
interface UserVideoGridProps {
  ...
  className?: string;
  /** Section heading. Pass `null` to render the grid with no heading (when the
   *  parent supplies its own, e.g. the public profile's "Popular Creations"). */
  heading?: string | null;
}

/** A single archive/Kine card. Extracted so it can be reused outside the grid
 *  (e.g. the mobile carousel on the public profile). */
export const VideoCard: React.FC<{ video: VideoType; className?: string }> = ({
  video,
  className = "",
}) => {
  ...
  return (
    <div
      ...
      className={`card-glass-panel rounded-xl overflow-hidden group cursor-pointer aspect-[9/16] relative flex flex-col justify-end transition-transform duration-500 hover:-translate-y-2 hover:glow-primary-lg border border-hairline/5 hover:border-primary/50 ${className}`}
    >
...
export const UserVideoGrid: React.FC<UserVideoGridProps> = ({
  userVideos,
  className = "mt-24 pb-24",
  heading = "Featured Archives",
}) => {
    ...
    <div className={`w-full max-w-6xl mx-auto px-4 ${className}`}>
      ...
      {heading !== null && (
        <div className="flex items-center justify-between mb-6 border-b border-hairline/10 pb-2">
          <h2 className="text-2xl md:text-3xl font-syne font-bold text-on-surface">{heading}</h2>
        </div>
      )}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {userVideos.map((video, index) => (
          <VideoCard
            key={video.id}
            video={video}
            className={index === 2 ? "md:hidden lg:flex" : ""}
          />
        ))}
      </div>
    </div>
